...
if(this.flashElement&&this.flashPlayerVersionSupported()){return true;}else{return false;}},flashInit:function(){this.replaceWithFlash();this.element=this.flashElement;this.video.src="";var flashPlayerType=VideoJS.flashPlayers[this.options.flashPlayer];this.extend(VideoJS.flashPlayers[this.options.flashPlayer].api);(flashPlayerType.init.context(this))();},getFlashElement:function(){var children=this.video.children;for(var i=0,j=children.length;i<j;i++){if(children[i].className=="vjs-flash-fallback"){return children[i];}}},replaceWithFlash:function(){if(this.flashElement){if(window.flashHtml){var id=jQuery(this.box).find('video').attr('id');var $flashElement=jQuery(window.flashHtml[id]);jQuery(this.box).prepend($flashElement);jQuery(this.flashElement).remove();this.flashElement=$flashElement[0];}else{this.box.insertBefore(this.flashElement,this.video);}
this.video.style.display="none";}},flashPlayerVersionSupported:function(){var playerVersion=(this.options.flashPlayerVersion)?this.options.flashPlayerVersion:VideoJS.flashPlayers[this.options.flashPlayer].flashPlayerVersion;return VideoJS.getFlashVersion()>=playerVersion;}});VideoJS.flashPlayers={};VideoJS.flashPlayers.htmlObject={flashPlayerVersion:9,init:function(){return true;},api:{width:function(width){if(width!==undefined){this.element.width=width;this.box.style.width=width+"px";this.triggerResizeListeners();return this;}
...
return this.video.offsetHeight;},supportsFullScreen:function(){if(typeof this.video.webkitEnterFullScreen=='function'||typeof this.video.mozRequestFullScreen=='function'||typeof this.video.msRequestFullscreen=='function'){return true;}
return false;},html5EnterNativeFullScreen:function(){try{if(typeof this.video.webkitEnterFullScreen=='function'){this.video.webkitEnterFullScreen();}else if(typeof this.video.mozRequestFullScreen=='function'){this.video.mozRequestFullScreen();}else if(typeof this.video.msRequestFullscreen=='function'){this.video.msRequestFullscreen();}}catch(e){if(e.code==11){this.warning(VideoJS.warnings.videoNotReady);}}
...
var width=this.width();var height=this.height();this.poster.style.height=height.toString().slice(-1)=='%'?height:height+"px";this.poster.style.width=width.toString().slice(-1)=='%'?width:width+"px";},hidePoster:function(){if(!this.poster){return;}
...
window.VideoJS=window._V_=VideoJS;})(window);function mediatisSetupVideoJs(id,origWidth,origHeight,loop,disableControls){jQuery(document).ready(function(){var player;if(disableControls){player=VideoJS.setup(id,{useBuiltInControls:true});}else{player=VideoJS.setup(id);}
var $video=jQuery('#'+id);var $container=$video.parents('.mediatis-html5-video-figure');var $object=$container.find('.vjs-flash-fallback');var ratio=origHeight/origWidth;function resizeVideo(){var width=$container.width();if(width==0){return;}
var height=width*ratio;player.width(width);player.height(height);$video.css('height',height);$object.css('height',height);}
jQuery(window).resize(function(){resizeVideo();});$container.find('img').load(function(){resizeVideo();});setTimeout(resizeVideo,100);setTimeout(resizeVideo,250);setTimeout(resizeVideo,500);setTimeout(resizeVideo,1000);if(loop){player.onEnded(function(){player.play();});}});}
